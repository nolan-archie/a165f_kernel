#!/usr/bin/env bash
# fix-release-name.sh - cleaner GitHub Release titles (was:
# "susfs-dev build 20260715-susfs-dev-09dd3e1c3", now:
# "susfs-dev build - Jul 15, 2026 (09dd3e1c3)")
set -euo pipefail
    ...
    steps:
      ...
      - name: Send final Telegram ping with real download link
        ...
        run: |
          ...
          TEXT=$(printf '<b>Release published</b>\nBranch: %s\n<a href="%s">Download build</a>' \
            "${{ matrix.branch }}" \
            "${RELEASE_URL}")
...
write_build_main
git add -A
git commit -m "ci: cleaner release titles (drop redundant date/branch numbers)" || echo "(nothing to commit)"

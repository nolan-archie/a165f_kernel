...
show_map_vma(struct seq_file *m, struct vm_area_struct *vma)
{
	...
	int susfs_open_redirect_srcu_idx = -1;
#endif // #ifdef CONFIG_KSU_SUSFS_OPEN_REDIRECT
	...
	if (file) {
		...
		if (SUSFS_IS_INODE_OPEN_REDIRECT(inode)) {
			susfs_open_redirect_srcu_idx = srcu_read_lock(&susfs_srcu_open_redirect);
			if (!susfs_open_redirect_spoof_show_map_vma_srcu(inode, &ino, &dev, &spoofed_redirected_name)) {
				pgoff = ((loff_t)vma->vm_pgoff) << PAGE_SHIFT;
				goto orig_flow;
			}
			/* lookup failed: nothing to hold the lock for, release now */
			srcu_read_unlock(&susfs_srcu_open_redirect, susfs_open_redirect_srcu_idx);
			susfs_open_redirect_srcu_idx = -1;
		}
#endif // #ifdef CONFIG_KSU_SUSFS_OPEN_REDIRECT
...
orig_flow:
	...
	if (spoofed_redirected_name) {
		...
		seq_putc(m, '\n');
		/* pointer is into SUSFS-owned data, not ours to free -- just drop the SRCU read side */
		srcu_read_unlock(&susfs_srcu_open_redirect, susfs_open_redirect_srcu_idx);
		return;
	}
#endif // #ifdef CONFIG_KSU_SUSFS_OPEN_REDIRECT

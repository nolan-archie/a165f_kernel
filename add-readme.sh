#!/usr/bin/env bash
# add-readme.sh - adds a professional README.md (SUSFS integration, root
# manager split, automation/CI pipeline, credits) to both branches.
set -euo pipefail
...
for b in main susfs-dev; do
  ...
  write_readme
  git add -A
  git commit -m "docs: add project README (SUSFS integration, CI pipeline, credits)" || echo "(nothing to commit on $b)"
  git push origin "$b"
done
